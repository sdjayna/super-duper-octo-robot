...
- **Shared hatching library** – fill routines (serpentine, scanline, contour, skeleton, or none) can now be selected per drawing, and the helpers accept arbitrary polygons so future drawings inherit the same ink-friendly toolpaths.
- **Layer travel optimizer** – each color layer reorders its segments with a nearest-neighbor heuristic so pen-up moves shrink dramatically before the next polygon or segment starts.
...
- **Photo Triangles** – upload any reference image and the new mosaic drawing samples it, runs a point-weighted Delaunay triangulation, then hatches each triangle (serpentine/scanline/contour/skeleton/none via the global controls) with the nearest palette color so multi-pen plots can echo real photos.
