...
export class BouwkampConfig extends SizedDrawingConfig {
    constructor(params = {}) {
        ...
        this.fitToMargin = params.fitToMargin === true;
    }
    ...
    getBounds(context = {}) {
        ...
        const rawWidth = Number(context.paper?.width ?? this.width);
        const rawHeight = Number(context.paper?.height ?? this.height);
        const longer = Math.max(rawWidth, rawHeight);
        const shorter = Math.min(rawWidth, rawHeight);
        const isPortrait = context.orientation === 'portrait';
        const orientedWidth = isPortrait ? shorter : longer;
        const orientedHeight = isPortrait ? longer : shorter;
        const availableWidth = Math.max(orientedWidth - margin * 2, 1);
        const availableHeight = Math.max(orientedHeight - margin * 2, 1);
        return {
            minX: 0,
            minY: 0,
...
export function drawBouwkampCode(drawingConfig, renderContext) {
  ...
  const desiredSpacing =
    typeof drawingConfig.line?.spacing === 'number' ? drawingConfig.line.spacing : 2.5;
  const spacing = Math.max(0.1, desiredSpacing);
  const hatchStyle =
    typeof drawingConfig.line?.hatchStyle === 'string'
      ? drawingConfig.line.hatchStyle
      : 'serpentine';
  const { svg, builder } = createDrawingRuntime({ drawingConfig, renderContext });
    const fillPaper = Boolean(bouwkamp.fitToMargin);
  ...
  for (let rect = 0; rect < bouwkamp.order; rect++) {
    ...
    const hatchInset =
      typeof drawingConfig.line?.hatchInset === 'number'
    ...
    if (hatchStyle === 'none') {
      ...
      const points = generateSingleSerpentineLine(
        projectedRect,
        spacing,
        drawingConfig.line.strokeWidth,
        {
          inset: hatchInset,
          includeBoundary
        }
...
export const bouwkampControls = [
  {
    ...
    valueType: 'number',
    description:
      'Inset distance from each square edge before the hatch starts (creates a clean border)'
  },
  {
    id: 'fitToMargin',
    label: 'Fit to Margin',
    target: 'drawingData.fitToMargin',
    inputType: 'checkbox',
    default: false,
    description:
      'Check to fit the rectangle to the paper margins. Uncheck to keep the original Bouwkamp proportions.'
  }
];

export const bouwkampDrawing = attachControls(
  defineDrawing({
    id: 'bouwkamp',
    name: 'Bouwkamp Code',
