...
const LISSAJOUS_LIMITS = {
    freqA: { min: 3, max: 9, default: 5 },
    freqB: { min: 3, max: 9, default: 7 },
    phase: { min: 0.05, max: 0.6, default: 0.2 },
    amplitude: { min: 0.4, max: 1, default: 0.9 },
    samples: { min: 2000, max: 4000, default: 3200 }
};
...
export function drawLissajous(drawingConfig, renderContext) {
    ...
    const { freqA, freqB, phase, amplitude, samples } = drawingConfig.drawingData;
    ...
    for (let i = 0; i <= samples; i++) {
        ...
        const x = centerX + Math.sin(freqA * t + phase) * scaleX;
        const y = centerY + Math.sin(freqB * t) * scaleY;
        points.push({ x, y });
    }
